...
public class TabList {
    ...
    private static void schedule() {
        ...
        scheduledFuture = ServiceProvider.scheduler.scheduleAtFixedRate(TabList::updateTab, 0, ServiceProvider.config().customTabList.tabListDelay, TimeUnit.MILLISECONDS);
    }

    public static void updateTab() {
        var period = Math.max(ServiceProvider.config().customTabList.tabPhasePeriod, 1);
        ...
        server.getPlayerManager().getPlayerList().forEach(player -> {
            ...
            var header = String.join("\n", ServiceProvider.config().customTabList.tabHeader);
            var footer = String.join("\n", ServiceProvider.config().customTabList.tabFooter);
