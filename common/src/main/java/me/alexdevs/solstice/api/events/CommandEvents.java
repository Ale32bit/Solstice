...
public class CommandEvents {
    public static final Event<AllowCommand> ALLOW_COMMAND = EventFactory.createArrayBacked(
            AllowCommand.class, callbacks -> (source, command) -> {
                for (var callback : callbacks) {
                    if (!callback.allowCommand(source, command)) return false;
                }
                return true;
            }
    );

    public static final Event<Command> COMMAND = EventFactory.createArrayBacked(
            Command.class, callbacks -> (player, command) -> {
                for (var callback : callbacks) {
