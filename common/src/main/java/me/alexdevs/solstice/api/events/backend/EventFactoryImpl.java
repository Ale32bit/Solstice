package me.alexdevs.solstice.api.events.backend;
...
public final class EventFactoryImpl {
    private static final Set<ArrayBackedEvent<?>> ARRAY_BACKED_EVENTS = Collections.newSetFromMap(new MapMaker().weakKeys()
            .makeMap());
    ...
    private static <T> T buildEmptyInvoker(Class<T> handlerClass, Function<T[], T> invokerSetup) {
        ...
        for (Method m : handlerClass.getMethods()) {
            if ((m.getModifiers() & (Modifier.STRICT | Modifier.PRIVATE)) == 0) {
                if (funcIfMethod != null) {
                    throw new IllegalStateException("Multiple virtual methods in " +
                                                    handlerClass +
                                                    "; cannot build empty invoker!");
                }
        ...
        return (T) Proxy.newProxyInstance(
                EventFactoryImpl.class.getClassLoader(),
