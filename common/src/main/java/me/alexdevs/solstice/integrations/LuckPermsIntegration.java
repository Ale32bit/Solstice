...
import me.alexdevs.solstice.api.permissions.OfflinePermissionCheckEvent;
import me.alexdevs.solstice.api.permissions.PermissionCheckEvent;
import me.alexdevs.solstice.api.permissions.TriState;
...
public class LuckPermsIntegration {
    ...
    public static void register() {
        if (!isAvailable()) {
            Solstice.LOGGER.warn(
                    "LuckPerms not available! It is recommended to install LuckPerms to configure permissions and groups.");
            return;
        }
        ...
        ProxyServerLifecycleEvents.SERVER_STARTED.register(server -> {
            ...
            if (!ConnectorIntegration.isForge()) {
                var container = PlatformHelper.get().getModContainer();
    ...
    public static boolean isAvailable() {
        return PlatformHelper.get().isModLoaded("luckperms");
    }
    ...
    public static @Nullable String getPrefix(ServerPlayer player) {
        ...
        return prefixMap.computeIfAbsent(
                player.getUUID(), uuid -> {
                    try {
    ...
    public static @Nullable String getSuffix(ServerPlayer player) {
        ...
        return suffixMap.computeIfAbsent(
                player.getUUID(), uuid -> {
                    try {
                        var playerMeta = luckPerms.getPlayerAdapter(ServerPlayer.class).getMetaData(player);
    ...
    public static CompletableFuture<Tristate> checkPermission(UUID uuid, String permission) {
        ...
        if (luckPerms.getUserManager().isLoaded(uuid)) {
            ...
            if (user == null) {
                ...
                future.complete(user.getCachedData().getPermissionData().checkPermission(permission));
            }
        } else {
            luckPerms.getUserManager()
                    .loadUser(uuid)
                    .thenAccept(user -> future.complete(user.getCachedData()
                            .getPermissionData()
