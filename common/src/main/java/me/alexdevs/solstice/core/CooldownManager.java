...
public class CooldownManager {
    ...
    public CooldownManager() {
        ProxyServerPlayConnectionEvents.JOIN.register((player, server) -> cooldowns.computeIfAbsent(
                player.getUUID(),
    ...
    public boolean onCooldown(ServerPlayer player, String node) {
        if (isExempt(player, node)) return false;
    ...
    public Component getMessage(ServerPlayer player, String node) {
        ...
        return locale.get("~cooldown", Map.of("timespan", Component.nullToEmpty(TimeSpan.toShortString(value))));
    }
