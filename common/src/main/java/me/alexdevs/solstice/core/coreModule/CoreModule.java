...
public class CoreModule extends ModuleBase {
    ...
    public void init() {
        ...
        ProxyServerPlayConnectionEvents.JOIN.register((player, server) -> {
            Solstice.getUserCache().add(player.getGameProfile());
            ...
            playerData.ipAddress = player.getIpAddress();
            ...
            if (playerData.username != null && !playerData.username.equals(player.getGameProfile().getName())) {
                Solstice.LOGGER.info(
                        "Player {} has changed their username from {}",
        ...
        ProxyServerPlayConnectionEvents.DISCONNECT.register((player, client) -> {
            var playerData = Solstice.playerData.get(player).getData(CorePlayerData.class);
