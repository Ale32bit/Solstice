...
public class MiscellaneousModule extends ModuleBase.Toggleable {
    ...
    public void init() {
        ...
        ProxyServerPlayConnectionEvents.JOIN.register((player, server) -> commandSleeping.remove(player.getUUID()));
        ProxyEntitySleepEvents.STOP_SLEEPING.register((entity, pos) -> commandSleeping.remove(entity.getUUID()));

        ProxyEntitySleepEvents.ALLOW_SLEEP_TIME.register((player, pos, vanillaResult) -> {
        ...
        ProxyEntitySleepEvents.ALLOW_RESETTING_TIME.register(player -> {
            if (commandSleeping.getOrDefault(player.getUUID(), false)) {
