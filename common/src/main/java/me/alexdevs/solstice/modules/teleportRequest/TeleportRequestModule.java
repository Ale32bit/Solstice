...
public class TeleportRequestModule extends ModuleBase.Toggleable {
    ...
    public void init() {
        ...
        ProxyServerPlayConnectionEvents.JOIN.register((player, server) -> requests.put(
                player.getUUID(),
                new ConcurrentLinkedDeque<>()
        ));

        ProxyServerPlayConnectionEvents.DISCONNECT.register((player, server) -> Solstice.nextTick(() -> requests.remove(
                player.getUUID())));
    }
    ...
    public void acceptRequest(ServerPlayer player, Request request) {
        ...
        var map = Map.of("player", player.getDisplayName());
    ...
    public void requestTo(ServerPlayer source, ServerPlayer target) {
        ...
        var placeholders = Map.of(
                "requesterPlayer", source.getDisplayName(), "acceptButton", Components.button(
                        locale().raw("~accept"),
        ...
        target.sendSystemMessage(locale().get("pendingTeleport", targetContext, placeholders));

        source.sendSystemMessage(locale().get("requestSent", sourceContext));
    ...
    public void requestToHere(ServerPlayer source, ServerPlayer target) {
        ...
        target.sendSystemMessage(locale().get("pendingTeleportHere", targetContext, placeholders));
